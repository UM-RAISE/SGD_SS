...
imperfectSave = "im1000TrainSASGDBDEx2Ver4.csv"
...
directFindLocal1SGDnonAdap <- function(data, start){
  ...
  for (j in c(1:iter)){
    ...
    for (k in c(1:length(start))){
      ...
      while (innerLeft>innerRight){
        ...
        newSamNum = min(startSamNum*addPercent+samNum, sampleLimit)
  ...
  newList <- list("thetaChoice" = thetaChoice, "sampleSize" = sampleSize, "proportion" = samNumMat, "paras" = paras1, "iters" = actualIter)
  return (newList)
}
