...
imperfectSave = "1000TrainSSGDCART2dUpdate.csv"
...
#      }
      ...
      for (ii in c(1:(numStra-1))){
        ...
        straIndex[ii+1] = straIndex[ii]+length(samDataSet[[ii+1]]$y)
      }
...
#    powerHat = -(data$x1-imThetaM[i,j]/2)^4-(data$x2-imThetaM[i,j])^2+4
    ...
    imRMSEM[i,j] = sqrt(sum((powerHat - data$y)^2)/length(powerHat))
  }
  ntm = proc.time() - ptm
...
#  powerHat = -(testData$x1-imTheta[i]/2)^4-(testData$x2-imTheta[i])^2+4
